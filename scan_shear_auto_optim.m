...
%% Plot shear without end (too high- for triang +)
newR10 = R10(1:27,:);
newR11 = R11(1:27,:);
newS   = S(1:27,:);
figure
pcolor(newR11,newR10,newS)
shading interp
hold on 
contour(newR11,newR10,newS,[0,0], 'r', 'LineWidth', 3)
set (gca, 'fontsize', fs)
colorbar
xlabel('$R_{11}$', 'interpreter', 'latex')
ylabel('$R_{10}$', 'interpreter', 'latex')

%% Plot shear along R10 (R11 fixed) %%
shear_trunc  = shear_mat(49,:);
scan10_trunc = R10(49,:);

shear_trunc2  = shear_mat(18,:);
scan10_trunc2 = R10(18,:);

level0 = zeros(1,length(scan10_trunc));

figure
%---------------------------------------------------------
subplot(1,2,2)
plot(scan10_trunc,shear_trunc,'b-','marker', '+', 'linewidth', 2)
hold on 
plot(scan10_trunc,level0, 'k--', 'linewidth', 1)
set (gca, 'fontsize', fs)
title('Shear $s(R_{11}=0.2828,R_{10})$', 'interpreter', 'latex')
xlabel('$R_{10}$', 'interpreter', 'latex', 'FontSize',28)
ylabel('$s$', 'interpreter', 'latex','FontSize',28)
%---------------------------------------------------------
subplot(1,2,1)
plot(scan10_trunc2,shear_trunc2,'b-','marker', '+', 'linewidth', 2)
hold on 
plot(scan10_trunc2,level0, 'k--', 'linewidth', 1)
set (gca, 'fontsize', fs)
title('Shear $s(R_{11}=0.1273,R_{10})$', 'interpreter', 'latex')
xlabel('$R_{10}$', 'interpreter', 'latex','FontSize',28)
ylabel('$s$', 'interpreter', 'latex','FontSize',28)
...
plot(scan11_trunc2, scan10_trunc2, 'w--', 'linewidth', 1);
hold on
plot(xpos_1, y_pos_1, 'kx', 'Linewidth', 2, 'MarkerSize', 20)
hold on
plot(xpos_1, y_pos_2, 'kx', 'Linewidth', 2, 'MarkerSize', 20)
set (gca, 'fontsize', fs)
colorbar
xlabel('$R_{11}$', 'interpreter', 'latex')
ylabel('$R_{10}$', 'interpreter', 'latex')

%% PCOLOR_shear 2 plots%%
scan11_trunc = R11(18,:);
scan11_trunc2 = R11(49,:);
figure
subplot(2,1,1)
